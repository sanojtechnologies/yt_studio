...
- **Light-mode remap contract.** The codebase is dark-by-default and ships hard-coded `bg-zinc-*` / `text-zinc-*` utilities without `dark:` variants. `app/globals.css` carries an `html:not(.dark)` block that remaps every zinc shade + slash-opacity tier actually used in the codebase to a light counterpart. Whenever a new `bg-zinc-{shade}/{alpha}` or `text-zinc-{shade}` utility is introduced, the corresponding override MUST be added to that block in the same change — otherwise the new surface will appear as a dark glass overlay in light mode. Currently covered background tiers: `bg-zinc-950`, `/70`, `/60`, `/50`; `bg-zinc-900`, `/95`, `/85`, `/80`, `/70`, `/60`, `/50`; `bg-zinc-800`, `/70`, `/60`; `bg-zinc-700/40`.
...
| Date       | Author        | Change                                                                                 |
|------------|---------------|----------------------------------------------------------------------------------------|
| 2026-05-12 | Light-mode remap completion | Extended the `html:not(.dark)` block in `app/globals.css` so every zinc slash-opacity tier actually used in the codebase has a light-palette counterpart. Closed the previously unmapped tiers (`bg-zinc-950/60`, `bg-zinc-900/{50,60,85,95}`, `bg-zinc-800/60`) that were causing globally fixed elements like the `PoweredByLink` badge — and section cards in TitleTrends / PublishHeatmap / DashboardInsights / dashboard skeletons — to render as dark glass overlays on the light theme. Updated PRD § 4.10 "Theme toggle" with the explicit remap contract enumerating every covered tier. No public API or component-level change; CSS-only. |
